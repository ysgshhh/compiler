...
MachineOperand* Instruction::genMachineOperand(Operand* ope)//生成操作数的汇编代码
{
...
void LoadInstruction::genMachineCode(AsmBuilder* builder)
{
    ...
    && dynamic_cast<IdentifierSymbolEntry*>(operands[1]->getEntry())->isGlobal())//将全局变量或常量加载到寄存器中
    {
        ...
        cur_inst = new LoadMInstruction(cur_block, internal_reg1, src);//第一步先将地址加载到寄存器
        cur_block->InsertInst(cur_inst);
        // example: load r1, [r0]
        cur_inst = new LoadMInstruction(cur_block, dst, internal_reg2);//第二步再将寄存器加载到目的
        cur_block->InsertInst(cur_inst);
    }
    // Load local operand
...
void StoreInstruction::genMachineCode(AsmBuilder* builder)
{
    ...
    if (operands[0]->getEntry()->isTemporary() && operands[0] -> getDef() && operands[0]->getDef()->isAlloc())//存在偏移时 
    {
    ...
    else if (operands[0]->getEntry()->isVariable() && t -> isGlobal()) //存储全局变量
    {
        MachineOperand *temp = genMachineVReg();
